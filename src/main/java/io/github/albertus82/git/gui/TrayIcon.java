...
import io.github.albertus82.git.gui.listener.EnhancedTrayRestoreListener;
...
public class TrayIcon implements IShellProvider, Multilanguage {
	...
	public static final String SHELL_WAS_MAXIMIZED = GitAutoSyncGui.class.getName() + ".wasMaximizedWhenMinimizedOnTray";
	...
	private Image image;
	private String tooltipText = GitAutoSyncGui.getApplicationName();

	TrayIcon(@NonNull final GitAutoSyncGui gui) {
		shell = gui.getShell();
	...
	private void iconify(final GitAutoSyncGui gui) {
	...
	public void showBalloonTooltip(@NonNull final Object o) {
		if (trayItem != null && !trayItem.isDisposed()) {
			...
			try {
				...
				log.warn("Cannot show balloon tooltip for " + o + ':', e);
			}
		}
	}

	public void updateTooltipText(final Object o) {
		final StringBuilder buf = new StringBuilder(GitAutoSyncGui.getApplicationName());
		if (o != null) {
		...
		if (trayItem != null && !trayItem.isDisposed()) {
			try {
				...
				log.warn("Cannot update tooltip text for " + o + ":", e);
			}
		}
	}
