...
import io.github.albertus82.git.config.logging.LoggingConfigAccessor;
import io.github.albertus82.git.config.logging.LoggingManager;
import io.github.albertus82.git.resources.LanguageManager;
...
public class GitAutoSyncConfig extends Configuration {
	...
	private static volatile GitAutoSyncConfig instance; // NOSONAR Use a thread-safe type; adding "volatile" is not enough to make this field thread-safe. Use a thread-safe type; adding "volatile" is not enough to make this field thread-safe.
	...
	private GitAutoSyncConfig() throws IOException {
	...
	private static GitAutoSyncConfig getInstance() {
		if (instance == null) {
			synchronized (GitAutoSyncConfig.class) {
				if (instance == null) { // The field needs to be volatile to prevent cache incoherence issues
					try {
						instance = new GitAutoSyncConfig();
						if (++instanceCount > 1) {
	...
	public static void initialize() {
		try {
			final GitAutoSyncConfig config = getInstance();
